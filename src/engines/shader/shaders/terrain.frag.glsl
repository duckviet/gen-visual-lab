...
void main() {
  ...
  vec2 movingUv = uv * (3.0 + u_swirl) + vec2(u_time * u_speed * 0.1, 0.0);
  float ridge = 1.0 - abs(snoise(movingUv + fbm(uv * 2.0) * u_distortion));
  float height = clamp(ridge * 0.6 + fbm(movingUv * 4.0) * 0.3, 0.0, 1.0);
  vec3 finalColor = samplePalette(height);
  finalColor += vec3((rand(uv + u_time) - 0.5) * u_grain * 0.2);
